...
import io.airbyte.integrations.destination.bigquery.operation.BigQueryGcsStorageOperations
...
class BigQueryDestination : BaseConnector(), Destination {
    ...
    private fun checkGcsPermission(config: JsonNode): AirbyteConnectionStatus? {
        ...
        val bucketName = loadingMethod[BigQueryConsts.GCS_BUCKET_NAME].asText()
        ...
        try {
            ...
            val storage =
                StorageOptions.newBuilder()
                    .setProjectId(config[BigQueryConsts.CONFIG_PROJECT_ID].asText())
    ...
    override fun getSerializedMessageConsumer(
        ...
        val bigquery = getBigQuery(config)
        val rawNamespaceOverride = getRawNamespaceOverride(RAW_DATA_DATASET)

        addAllStringsInConfigForDeinterpolation(config)
        val serviceAccountKey = config[BigQueryConsts.CONFIG_CREDS]
        ...
        if (uploadingMethod == UploadingMethod.STANDARD) {
            return BigQueryRecordStandardConsumer.createStandardConsumer(
                bigquery,
                config,
                catalog,
                parsedCatalog,
        ...
        val syncOperation: SyncOperation =
            DefaultSyncOperation<BigQueryDestinationState>(
                ...
                { destinationInitialStatus: DestinationInitialStatus<BigQueryDestinationState> ->
                    StagingStreamOperations(bigQueryGcsStorageOperations, destinationInitialStatus, disableTypeDedupe)
                },
                listOf()
            )
    ...
    companion object {
        private const val RAW_DATA_DATASET = "raw_data_dataset"
